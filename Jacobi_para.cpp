...
//LC, of problem
  ...
  while((l<=L)&&(n_res/n_res_0 > epsilon)){
    ...
    if (myRank < nbTasks-1){
      ...
      MPI_Irecv(&sol[(i_end-1) + (Nx+2)*(Ny+2)], 1, MPI_DOUBLE, myRank+1, 0, MPI_COMM_WORLD, &reqRecvRight);
    }
    ...
    for (int i=i_start; i<=i_end; i++){
        for (int j=1; j<=Ny; j++){
            solNew[i+(Nx+2)*j + 1] = coeff * ((sol[i+1+(Nx+2)*j+1]+sol[i-1+(Nx+2)*j + 1])/(dx*dx) + (sol[i+(Nx+2)*(j+1) + 1]+sol[i+(Nx+2)*(j-1) + 1])/(dy*dy) - f[i + (Nx+2)*j + 1]);     
            n_res += (solNew[i+(Nx+2)*j + 1] - sol[i+(Nx+2)*j + 1])*(solNew[i+(Nx+2)*j + 1] - sol[i+(Nx+2)*j + 1]);
        }
    }
    ...
    double buff = n_res;
    MPI_Reduce (&buff , &n_res , 1, MPI_DOUBLE , MPI_SUM , 0, MPI_COMM_WORLD ); //n_res has the right value
