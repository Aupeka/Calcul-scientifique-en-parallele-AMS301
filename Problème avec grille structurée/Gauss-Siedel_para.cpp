...
//LC, of problem
  ...
  while (l < 1){
    ...
    for (int i=1; i<=Nx; i++){
        for (int j=1; j<=Ny; j++){
            solNew[i+(Nx+2)*j + 1] = coeff * ((sol[i+1+(Nx+2)*j+1]+solNew[i-1+(Nx+2)*j + 1])/(dx*dx) + (sol[i+(Nx+2)*(j+1) + 1]+solNew[i+(Nx+2)*(j-1) + 1])/(dy*dy) - f[i + (Nx+2)*j + 1]);     
  ...
  while((l<=L)&&(n_res/n_res_0 > epsilon)){
    ...
    for (int i=1; i<=Nx; i++){
      for (int j=1; j<=Ny; j++){
        ...
        if ((i+j)%2==1){ //if black
         solNew[i*(Ny+2) + j] = (-(solNew[(i+1)*(Ny+2) + j] + solNew[(i-1)*(Ny+2) + j])/(dx*dx) - (solNew[i*(Ny+2) + j+1] + solNew[i*(Ny+2) + j-1])/(dy*dy) + f[i*(Ny+2) + j])/(-2/(dx*dx)-2/(dy*dy));
          n_res += (solNew[i*(Ny+2) + j] - sol[i*(Ny+2) + j])*(solNew[i*(Ny+2) + j] - sol[i*(Ny+2) + j]);
        }  
      }
  ...
  // Show results
    ...
    //Print
    if (myRank==0){
      cout << "***** Printing Gauss-Siedel's solution *****" <<endl;
      ...
      if (l > L){
        ...
        cout << "Program stopped because the residual value = " << n_res/n_res_0 << "< "<< epsilon << "and l = " << l << endl;
      }
    ...
    if (myRank==0){
    ofstream file;
    file.open("GS_para_cholesky.txt", ios::app);
